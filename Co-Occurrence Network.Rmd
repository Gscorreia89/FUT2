...
# Setup color labels
```{r}
labelList_Secretor <- list(`L. crispatus`=which(colnames(secretor_res) == 'L. crispatus'), 
     `L. iners`=which(colnames(secretor_res) == 'L. iners'),
     `L. gasseri`=which(colnames(secretor_res) == 'L. gasseri'), 
     `L. jensenii`=which(colnames(secretor_res) == 'L. jensenii'),
      `G. vaginalis`=which(colnames(secretor_res) == 'G. vaginalis'),
     Lactobacillus=which(colnames(secretor_res) %in% c('L. vaginalis', 'L. coleohominis', 'Lactobacillus')),
     BV=which(colnames(secretor_res) %in% c('Anaerococcus', 'A. vaginae', 'P. bivia', 'P. corporis', 'P. anaerobius', 'P. colorans', 'P. disiens', 'S. amnii', 'Megasphaera', 'P. buccalis', 'P. timonensis', 'P. uenonis', 'A. christensenii', 'Peptoniphilus', 'A. schaalii', 'V. cambriense', 'F. massiliensis', "D. micraerophilus", "D. propionicifaciens", "Dialister", "V. montpellierensis", 
      "A. hydrogenalis", "A. lactolyticus",  "Anaerococcus",  'F. magna', "P. coxii", "P. indolicus", "P. lacrimalis", "Peptoniphilus", 'U. parvum', 'F. nucleatum')),
     Pathobiont=which(colnames(secretor_res) %in% c('S. agalactiae', 'S. anginosus')),
     `B. breve`=which(colnames(secretor_res) %in% c('B. breve')))

labelList_NSecretor <- list(`L. crispatus`=which(colnames(nsecretor_res) == 'L. crispatus'), 
     `L. iners`=which(colnames(nsecretor_res) == 'L. iners'),
     `L. gasseri`=which(colnames(nsecretor_res) == 'L. gasseri'), 
     `L. jensenii`=which(colnames(nsecretor_res) == 'L. jensenii'),
     `G. vaginalis`=which(colnames(nsecretor_res) == 'G. vaginalis'),
     Lactobacillus=which(colnames(nsecretor_res) %in% c('L. vaginalis', 'L. coleohominis', 'Lactobacillus')),
     BV=which(colnames(nsecretor_res) %in% c('Anaerococcus', 'A. vaginae', 'P. bivia', 'P. corporis', 'P. anaerobius', 'P. colorans', 'P. disiens', 'S. amnii', 'Megasphaera', 'P. buccalis', 'P. timonensis', 'P. uenonis', 'A. christensenii', 'Peptoniphilus', 'A. schaalii', 'V. cambriense', 'F. massiliensis', "D. micraerophilus", "D. propionicifaciens", "Dialister", "V. montpellierensis", 
      "A. hydrogenalis", "A. lactolyticus",  "Anaerococcus",  'F. magna', "P. coxii", "P. indolicus", "P. lacrimalis", "Peptoniphilus", 'U. parvum', 'F. nucleatum')),
     Pathobiont=which(colnames(nsecretor_res) %in% c('S. agalactiae', 'S. anginosus')),
     `B. breve`=which(colnames(nsecretor_res) %in% c('B. breve')))

# Similar structure but for ggraph
nodeColorsCustom <- c()
for (x in names(labelList_Secretor)) {
  for (y in labelList_Secretor[[x]]) {
    nodeColorsCustom <- rbind(nodeColorsCustom, c(y, x))
  }
}

nodeColorsCustom <- data.frame(nodeColorsCustom)
colnames(nodeColorsCustom) <- c('label', 'Type')
nodeColorsCustom$label <- as.numeric(nodeColorsCustom$label)
nodeColorsCustom$Type <- factor(nodeColorsCustom$Type, levels=c('L. crispatus', 'L. gasseri', 'L. iners', 'L. jensenii', 'G. vaginalis', 'Lactobacillus', 'BV', 'Pathobiont', 'Other'))
```
...
secretor_EIPlot <- ggplot(data=sec_cent_df, aes(x=Taxon, y=ExpectedInfluence)) + geom_bar(stat='identity', fill="cyan", alpha=0.5)  + theme_light() + scale_y_continuous(name="Expected Influence") + coord_flip() + xlab(NULL) + theme(axis.text.y = element_blank())

nonsecretor_EIPlot <- ggplot(data=nsec_cent_df, aes(x=Taxon, y=ExpectedInfluence)) + geom_bar(stat='identity', fill="red", alpha=0.5)  + theme_light() + scale_y_continuous(name="Expected Influence") + coord_flip() + theme(axis.text.y=element_text(face=c("italic")))
...
sec_qgrp <- qgraph::qgraph(secretor_res, layout="spring", vsize=4, border.width=1, groups=labelList_Secretor, color=c("#4daf4a","#ff7f00", "#377eb8","#ffff33","#2134ca", "#a65628", "#984ea3", "#e41a1c", "#666666"),labels=T)
nsec_qgrp <- qgraph::qgraph(nsecretor_res, layout="spring", vsize=4, border.width=1, groups=labelList_NSecretor, color=c("#4daf4a","#ff7f00", "#377eb8","#ffff33","#2134ca", "#a65628", "#984ea3", "#e41a1c", "#666666"), labels=T)
```

# Non-secretor network plot
```{r}
CustomColorsValues=c("L. crispatus"="#4daf4a", "L. iners"="#ff7f00", "L. gasseri"="#377eb8", "G. vaginalis"="deeppink", 
                     "L. jensenii"="#ffff33", "tomato3", "Lactobacillus"="#a65628", "BV"="#984ea3", "Pathobiont"="#e41a1c", "B. breve"="#666666")

# plot using ggraph
graph_tbl_nsec <- nsec_qgrp %>% 
  as_tbl_graph() %>% 
  activate(nodes) %>% left_join(nodeColorsCustom, by = join_by(label == label))
...
networkNonSecretor <- ggraph(layout_nsec) +
  geom_edge_fan(
    ...
    show.legend = F
  ) +  geom_node_circle(aes(fill = Type, r=0.075),  show.legend = T, color='black') + scale_fill_manual(values=CustomColorsValues)  + scale_edge_colour_gradient2(low = "firebrick3",
  mid = "white", high = "darkgreen", midpoint = 0,  space = "Lab", na.value = "grey50",guide = "edge_colourbar",
  aesthetics = "edge_colour") +  theme_graph() + scale_edge_width(range = c(0, 2), guide = "none") + theme(legend.text = element_text(size = 10,  family="sans-serif"), legend.title=element_text(size=12,  family="sans-serif"))

```

# Secretor network plot
```{r}
CustomColorsValues=c("L. crispatus"="#4daf4a", "L. iners"="#ff7f00", "L. gasseri"="#377eb8",
                     "L. jensenii"="#ffff33", "G. vaginalis"="deeppink", "Lactobacillus"="#a65628", "BV"="#984ea3", "Pathobiont"="#e41a1c", "Other"="#666666")
...
networkPanel <- (networkSecretor | networkNonSecretor ) + plot_layout(guides = 'collect') & theme(plot.margin =  unit(c(0,0,0,0), 'cm'))
...
bugIdx <- which(colnames(secretor_res) == 'L. crispatus')
flow(sec_qgrp, bugIdx, vsize=3)
flow(nsec_qgrp, bugIdx, vsize=3)
#flow diagram for L.gasseri
bugIdx <- which(colnames(secretor_res) == 'L. gasseri')
flow(sec_qgrp, bugIdx, vsize=3)
flow(nsec_qgrp, bugIdx, vsize=3)
#flow diagram for L.iners
bugIdx <- which(colnames(secretor_res) == 'L. iners')
flow(sec_qgrp, bugIdx, vsize=3)
flow(nsec_qgrp, bugIdx, vsize=3)
#flow diagram for L.jensenii
bugIdx <- which(colnames(secretor_res) == 'L. jensenii')
flow(sec_qgrp, bugIdx, vsize=3)
flow(nsec_qgrp, bugIdx, vsize=3)
#flow diagram for G. vaginalis
bugIdx <- which(colnames(secretor_res) == 'G. vaginalis')
flow(sec_qgrp, bugIdx, vsize=3)
flow(nsec_qgrp, bugIdx, vsize=3)
```
...
starSec_LCrisp <- ggraph(layout_sec_crisp) +
  ...
  geom_node_point(
    ...
    show.legend = T
  ) + scale_color_manual(values=CustomColorsValues)  + scale_edge_colour_gradient2(low = "firebrick3",
  mid = "white",high = "darkgreen", midpoint = 0,  space = "Lab", na.value = "grey50",guide = "edge_colourbar",
  aesthetics = "edge_colour") +  theme_graph() + scale_edge_width(range = c(0, 2), guide = "none") + theme(legend.text = element_text(size = 10,  family="sans-serif"), legend.title=element_text(size=12,  family="sans-serif"))  +  theme(plot.margin = margin(0, 0, 0, 0, "pt"))
...
starDiagramFigures <- starDiagramFigures + plot_layout(guide='collect')
...
networkFigure <- networkFigure + plot_layout(heights = c(1, 2)) 
